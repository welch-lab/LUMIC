...
class HTuple():
    def __init__(self, node=None, mess=None, vmask=None, emask=None):
        ...
        if emask is not None:
            self.emask = emask.cuda()
        else:
            self.emask = emask
        if vmask is not None:
            self.vmask = vmask.cuda()
        else:
            self.vmask = vmask
...
class HierMPNDecoder(nn.Module):
    ...
    def __init__(self, vocab, avocab, rnn_type, embed_size, hidden_size, latent_size, depthT, depthG, dropout, attention=False):
        ...
        self.hmpn = IncHierMPNEncoder(vocab, avocab, rnn_type, embed_size, hidden_size, depthT, depthG, dropout).cuda()
        self.rnn_cell = self.hmpn.tree_encoder.rnn.cuda()
        self.E_assm = self.hmpn.E_i.cuda()
    ...
    def decode(self, src_mol_vecs, greedy=True, max_decode_step=100, beam=10):
        src_root_vecs, src_tree_vecs, src_graph_vecs = src_mol_vecs
        batch_size = len(src_root_vecs)
        ...
        for t in range(max_decode_step):
            ...
            subtree = subtree[0].cuda(), batch_idx.new_tensor(new_mess).cuda()
            subgraph = [], []
            htree, hinter, hgraph = self.hmpn(tree_tensors, tree_tensors, graph_tensors, htree, hinter, hgraph, subtree, subgraph)
            # print(next(self.rnn_cell.parameters()).is_cuda)
            # print(self.rnn_cell.get_hidden_state(htree.mess).get_device())
            cur_mess = self.rnn_cell.get_hidden_state(htree.mess).cuda().index_select(0, subtree[1].cuda())
